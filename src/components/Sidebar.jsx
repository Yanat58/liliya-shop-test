...
const Sidebar = ({ onClose }) => {
  ...
  return (
    <div
      ...
      className="fixed top-0 left-0 w-screen h-screen z-20 bg-black/60"
    >
      <div>
        <button
          onClick={onClose}
        ...
        >
          <img
            src={close}
            alt="close"
            className="w-8 h-8 hover:color-red-600"
          />
        </button>
      </div>
      ...
      >
        <div
          className={`${styles.line} border-b-primary bg-dimBlack py-6 pl-6`}
        >
        ...
        <div
          className="bg-dimBlack py-6 pl-6"
          onClick={() => setShow(prev => !prev)}
        >
          <p
            className={`${styles.title} text-10 cursor-pointer text-black uppercase`}
          >
            Інформація
          </p>
        </div>
